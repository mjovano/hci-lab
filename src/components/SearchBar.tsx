...
export default function Search( { showButton }: SearchProps) {
  ...
  return (
    <form action="/shop" className="flex items-center justify-center w-full">
      ...
      <button
        type="submit"
        className={`group ml-2 px-4 py-2 bg-zinc-700 rounded-2xl hover:bg-zinc-900 shadow-md ${showButton === 0 ? 'hidden' : ''}`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className="size-6 fill-indigo-200 group-hover:fill-indigo-400"
        >
          <path
        fillRule="evenodd"
